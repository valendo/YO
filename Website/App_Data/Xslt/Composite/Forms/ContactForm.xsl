							...
							<ul id="contact_form">
								...
								<li class="clearfix">
									<xsl:variable name="SendText">
										<xsl:text>
											<lang:string key="Resource, Resources.ContactForm.SendButton" />
										</xsl:text>
									</xsl:variable>
									<input name="submitContactForm" id="submitContactForm" type="submit" class="send-btn" value="{normalize-space($SendText)}" />
								</li>
							</ul>
						</asp:form>
					</xsl:otherwise>
